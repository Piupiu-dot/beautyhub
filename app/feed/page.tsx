...
const SORTS = [
  { key: 'neu', label: 'Aktuell' },
  { key: 'beliebt', label: 'Beliebt' },
  { key: 'gesetz', label: 'Gesetze' },
  { key: 'trend', label: 'Trends' },
]
const DEMO: Post[] = [
  { id: '1', titel: 'Neue Laserklassen-Vorschriften 2026', zusammenfassung: 'Das Gesundheitsamt hat aktualisierte Richtlinien fuer Lasergeraete ab Klasse 3B erlassen. Kosmetikstudios benoetigen ab Juli eine erweiterte Zertifizierung.', typ: 'gesetz', nischen: 'laser', quelle_name: 'BAG Schweiz', likes: 24, ist_agent: true },
  ...
  { id: '3', titel: 'Bio-Fermentation: Der groesste Beauty-Trend 2026', zusammenfassung: 'Fermentierte Wirkstoffe gelten als der groesste Trend. Lacto-Fermentation verbessert die Bioverfuegbarkeit von Vitaminen.', typ: 'trend', nischen: 'gesicht', quelle_name: 'Cosmetica Suisse', likes: 41, ist_agent: true },
  { id: '4', titel: 'Neue Erkenntnisse zu Gel-Systemen bei Naegeln', zusammenfassung: 'Studie zeigt: Pausen zwischen Gel-Applikationen reduzieren Schaeden um 60 Prozent.', typ: 'news', nischen: 'nagel', quelle_name: 'SDKF', likes: 35, ist_agent: false },
]
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data }) => {
      if (!data.session) router.push('/login')
        ...
        const n = data.session.user.user_metadata?.name || data.session.user.email || 'M'
        setAvatar(n[0].toUpperCase())
      }
    })
  }, [router])
  ...
  return (
    <AppShell>
        ...
        <div className="flex gap-2 whitespace-nowrap">
          {NISCHEN.map(n => (
            <button key={n} onClick={() => setNische(n)}
              className={`px-4 py-2 rounded-full text-xs font-medium border-[1.5px] transition-all ${nische === n ? 'bg-[#1A1A2E] text-white border-[#1A1A2E]' : 'bg-white text-[#6B6B6B] border-[#E8E0D5]'}`}>
      ...
      <div className="p-4 grid gap-3 sm:grid-cols-2">
        {loading ? (
          <><div className="skeleton h-52"/><div className="skeleton h-52"/><div className="skeleton h-52"/></>
        ) : posts.length === 0 ? (
          <div className="col-span-2 text-center py-16 text-[#9A9A9A]">Keine Inhalte gefunden</div>
        ) : (
